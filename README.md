...
- **오프라인 정적 데모**: [`submission/demo.html`](./submission/demo.html) — 서버/LLM 호출 없이
  더블클릭만으로 브라우저에서 열리는 정적 화면입니다. 외부에 URL을 노출하지 않고 사이드바
  UI와 대화 흐름(검색 → 등록 대화 → 확인 카드)만 빠르게 확인하고 싶을 때 사용하세요.
...
- **목록 보기 / 삭제 / 변경 이력**: 사이드바의 "정책 목록" 메뉴로 등록된 정책/용어/이용약관을
  ...
  있습니다.
- **이전 대화 이어보기 / 삭제**: 사이드바에 최근 대화 목록이 뜨고, 클릭하면 그 대화를 이어서
  볼 수 있습니다. "새 대화" 버튼으로 언제든 새로 시작할 수 있고, 각 항목에 마우스를 올리면
  (터치 환경에서는 항상) 나타나는 더보기(⋯) 버튼 → 팝오버 메뉴에서 확인 후 개별 삭제도
  가능합니다 — 지금 보고 있던 대화를 지우면 새 대화 시작 화면으로 돌아갑니다. 목록 위에는
  보관기간과 등록 완료 방법을 안내하는 문구가 목록이 비어 있어도 항상 표시됩니다.
- **대화 세션 저장**: 로그인 사용자의 대화가 `data/chats/`에 날짜별로 저장되고, 24시간
  (`CHAT_RETENTION_HOURS`)이 지나면 자동으로 정리됩니다(위 사이드바 목록도 이 보관기간 안의
  대화만 보여줍니다).
...
### 4. 이전 대화 이어보기 / 삭제
1. 대화를 몇 개 진행한 뒤 사이드바 "새 대화"로 새로운 대화를 시작 → 사이드바 "이전 대화"
   목록에 방금 대화들이 최신순으로 쌓이는지 확인.
2. 목록의 항목을 클릭 → 그 대화가 그대로 이어서 보이는지(메시지 이력 복원) 확인.
3. 항목에 마우스를 올려 더보기(⋯) 버튼 → 팝오버 메뉴에서 "삭제" 클릭 → 확인 문구 노출 →
   확인 시 목록에서 즉시 사라지는지 확인. 지금 보고 있던 대화를 삭제한 경우 화면이 새 대화
   시작 전 초기 화면으로 바뀌는지 확인.
4. 목록 위 안내 문구(보관기간 24시간 안내)가 대화가 없을 때도 계속 보이는지 확인.
...
# 이전 대화 목록 / 특정 대화 조회 / 삭제
curl http://localhost:3000/api/chat/sessions -b cookies.txt
curl http://localhost:3000/api/chat/sessions/<sessionId> -b cookies.txt
curl -X DELETE http://localhost:3000/api/chat/sessions/<sessionId> -b cookies.txt
```
